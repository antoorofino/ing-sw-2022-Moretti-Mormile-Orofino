...
import it.polimi.ingsw.Model.AssistenceCard;
    ...
    */
    ...
    private void manageWin() throws Exception {
        ...
        if(first.getNumOfTower() != second.getNumOfTower()){
            ...
            List<Player> orderedByTeacherControl = orderedByTowers
                    .stream()
                    .filter(p -> game.getTeacherHandler().teachersControlled(p) == game.getTeacherHandler().teachersControlled(finalFirst))
                    .sorted((p1, p2) -> {
    ...
    public boolean setAction(Action action, String nickname) throws PlayerException {
        ...
        if(mode == GameMode.BASIC){
            try{
                //TODO: call doAction of basic rules
            } catch (Exception ignored){
                return false;
            }
        } else {
            try{
                ...
                //endImmediately = thePlayer.getActiveCharacter().getRules().doAction(action, game);
            } catch (Exception ignored){
                return false;
            }
        }
    ...
    public boolean setAssistantCard(String nickname, AssistenceCard card){
    ...
    private boolean checkAssistantCard(AssistenceCard card){
        List<AssistenceCard> alreadyPlayed = game.getPlayerHandler().cardsAlreadyPlayed();
        List<AssistenceCard> playerDeck = game.getPlayerHandler().getCurrentPlayer().getDeck();
        if(alreadyPlayed.size() >= playerDeck.size() && alreadyPlayed.containsAll(playerDeck))
            return true;
        for(AssistenceCard c : alreadyPlayed){
